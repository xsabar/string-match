...
static int _sttable_array_set(struct _sttable_array *tbl, int fid, char c, int tid) {
 ...
 */
static stlist_node_t* _sttable_hasht_get_node(struct _sttable_hasht *tbl, int id, char c, int h) {
 ...
 * @param tbl 表指针
 */
static void _sttable_hasht_rehash(struct _sttable_hasht *tbl) {
    int lnum = 0;
    int size = tbl->size;
    int base = tbl->base;
 ...
 */
static int _sttable_hasht_extend(struct _sttable_hasht *tbl) {
    int cap = tbl->cap * 2;
    int thrd = tbl->thrd * 2;
...
static int _sttable_hasht_set(struct _sttable_hasht *tbl, int fid, char c, int tid) {
...
static int _sttable_hasht_get(struct _sttable_hasht *tbl, int id, char c) {
    int h = _sttable_hasht_hash(id, c, tbl->base);
    stlist_node_t *node = _sttable_hasht_get_node(tbl, id, c, h);
    return node != NULL ? node->tid : -1;
}

inline int sttable_set(sttable_t *tbl, int fid, char c, int tid) {
    if (tbl->type == STTABLE_TYPE_ARRAY) {
...
inline int sttable_get(sttable_t *tbl, int id, char c) {
    if (tbl->type == STTABLE_TYPE_ARRAY) {
...
inline void sttable_copy(sttable_t *tbl, int fid, int tid) {
